...
async function getFeeData() {
	const response = await provider.getFeeData();

	var feeData = {
		maxFeePerGas : response.maxFeePerGas.toNumber(),
		maxPriorityFeePerGas : response.maxPriorityFeePerGas.toNumber()
	}
...
async function useControlToken(privateKey, contractAddress, tokenId, leverIds, newValues) {
	...
	var GAS_FEE_DATA = await getFeeData();
	...
	if (leverIds.length > 0) {
		var tx = await contractWithSigner.useControlToken(tokenId, leverIds, newValues, {
			maxFeePerGas : GAS_FEE_DATA.maxFeePerGas,
			maxPriorityFeePerGas : GAS_FEE_DATA.maxPriorityFeePerGas
		});
